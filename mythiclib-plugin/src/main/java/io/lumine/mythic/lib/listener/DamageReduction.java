...
//        player.sendMessage("DEF: " + data.getStatMap().getStat("DEFENSE"));
//        player.sendMessage("total DMG: " + event.getDamage().getDamage());
//        player.sendMessage("DMG_elems: " + event.getDamage().collectElements());
//        player.sendMessage("DMG_types: " + event.getDamage().collectTypes());
//        player.sendMessage("Neutral dmg: " + event.getDamage().getDamage((Element) null));

//        for (Element elem : event.getDamage().collectElements()) {
//            player.sendMessage(elem.getName() + " elem dmg: " + event.getDamage().getDamage(elem));
//        }

        if (defense > 0) {
            for (Element elem : event.getDamage().collectElements()) {
                double elemdmg = event.getDamage().getDamage(elem);
//                System.out.println(elemdmg);
                if (elemdmg > 0) {
                    final double ratio = Math.max(0, DefenseFormula.calculateDamage(true, defense, elemdmg)) / elemdmg;
//                    player.sendMessage(elem.getName() + " new elem dmg: " + elemdmg * ratio);
                    event.getDamage().multiplicativeModifier(ratio, elem);
                }
            }
        }
...
//            player.sendMessage("NEW neutral DMG: " + event.getDamage().getDamage((Element) null) * ratio);
